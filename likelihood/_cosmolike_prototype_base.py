...
class _cosmolike_prototype_base(_DataSetLikelihood):
  ...
  def get_datavector(self, **params_values):
    ...
    # setup systematic parameters of the lens sample
    if self.probe not in ['xi', 'CMBL', 'sk2x2pt', 'sk3x2pt']:
      self.set_lens_related(**params_values)
    # setup systematic parameters of the source sample
    if self.probe not in ['wtheta', 'CMBL', 'gk2x2pt', 'gk3x2pt']:
      self.set_source_related(**params_values)
