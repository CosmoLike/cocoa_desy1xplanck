...
#endif
  memcpy(cmb.pathLensRecNoise, cmb_noise_file.c_str(),
    cmb_noise_file.size()+1);

  like.lmax_kappacmb = 2999.;

  spdlog::info("\x1b[90m{}\x1b[0m: {} = {} selected.",
    "init_cmb", "cmb_noise_file", cmb_noise_file);
...
// real space binning
void cpp_init_binning_real(const int Ntheta, const double theta_min_arcmin,
  const double theta_max_arcmin) {
  spdlog::debug("\x1b[90m{}\x1b[0m: Begins", "init_binning_real");
#ifdef DEBUG
  if (!(Ntheta > 0)) {
    spdlog::critical("\x1b[90m{}\x1b[0m: {} = {} not supported",
      "init_binning_real", "like.Ntheta", Ntheta);
    exit(1);
  }
#endif
  spdlog::debug("\x1b[90m{}\x1b[0m: {} = {} selected.", "init_binning_real",
    "Ntheta", Ntheta);
  spdlog::debug("\x1b[90m{}\x1b[0m: {} = {} selected.", "init_binning_real",
  ...
  for (int i = 0; i < like.Ntheta; i++) {
    ...
    spdlog::debug(
      "\x1b[90m{}\x1b[0m: Bin {:d} - {} = {:.4e}, {} = {:.4e} and {} = {:.4e}",
      "init_binning_real", i, "theta_min [rad]", thetamin, "theta [rad]",
      like.theta[i], "theta_max [rad]", thetamax);
  }

  spdlog::debug("\x1b[90m{}\x1b[0m: Ends", "init_binning_real");
}
...
PYBIND11_MODULE(cosmolike_desy1xplanck_interface, m) {
    ...
    m.def("init_cmb", &cpp_init_cmb, "Init CMB", py::arg("cmb_noise_file"));
    ...
    m.def("init_binning_real", &cpp_init_binning_real,"Init Binning Real", py::arg("Ntheta"), py::arg("t_min"), py::arg("t_max"));
